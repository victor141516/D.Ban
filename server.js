...
if (IN_MEMORY) DATA = {}
    ...
    if (fs.existsSync(DB_NAME)) {
        ...
        console.info('Number of documents', Object.keys(DATA).length)
    } else {
        DATA = {}
        _writeToFile()
    }
...
http.createServer(function (req, res) {
    ...
    if (pathParts.length > 0 && pathParts[0] !== '' && !pathParts[0].startsWith('filter?') && !/([a-f0-9]{16})/.test(pathParts[0])) {
        res.statusCode = 400
        res.write(JSON.stringify({ error: 'invalid_key' }))
        return res.end()
    }
